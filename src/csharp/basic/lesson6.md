...
- A `char` típus karakterek tárolására szolgál.
...
A méretek és tartományok azt mutatják meg, mikor választhatjuk a `short` vagy a `long` típust az `int` helyett. Ha a memória korlátozott és a `short` tartománya elegendő, használhatjuk `short`-ot. Ha olyan számokat kell használnunk, amelyek nagyobbak, mint amit az `int` kezelne, akkor a `long` típusra kell váltanunk, még akkor is, ha több bájtba kerül.
...
Ha csak pozitív számokra van szükségünk, akkor erre szolgálnak az előjel nélküli típusok: `ushort`, `uint` és `ulong`. Ezek mindegyike ugyanannyi bájtot használ, mint az előjeles megfelelőjük, nem tárolhatunk bennük negatív számokat, de cserébe kétszer annyi pozitív számot tárolhatunk. Így az `ushort` tartománya __0__-tól körülbelül __65 000__-ig, az `uint` tartománya __0__-tól körülbelül __4__ milliárdig, az `ulong` tartománya pedig __0__-tól körülbelül __18__ kvintillióig terjed.

Az utolsó két egész számtípus egy kicsit más. A `byte` típus, amely egyetlen bájtot használ a __0__ és __255__ közötti értékek ábrázolására (_előjel nélküli_). Habár a byte-típust gyakrabban használják olyan bájtok gyűjteményének kifejezésére, amelyeknek nincs konkrét struktúrájuk (_vagy a program nem ismeri őket_). A bájt típusnak van egy előjeles megfelelője, az `sbyte`, amely a __-128__ és __+127__ közötti értékeket képviseli. Az `sbyte` típus nem túl gyakran használatos, de teljessé teszi a készletet.
...
Az ilyen más típusú változók deklarálása ugyanolyan egyszerű, mint az `int` vagy `string` típusok esetén, csak használjuk ezeknek a típusoknak a nevét `int` vagy `string` helyett, ahogyan azt korábban is tettük:
...
A __32__ egy szám literál, de a _fordító_ elég intelligens ahhoz, hogy észrevegye, hogy egy `byte`-ban próbáljuk tárolni. Ellenőrzi, hogy a __32__ megengedett tartományon belül van-e a `byte` típus esetében. Ellenkező esetben, ha olyan literált használnánk, ami túl nagy a `byte` típushoz, fordítási hibát kapnánk, ami megakadályozza a programunk fordítását és futtatását.

Ugyanez a szabály érvényes az `sbyte`, `short` és `ushort` típusokra is.
...
Előfordulhat, hogy néha azt szeretnénk, hogy egy kisebb szám valamely nagyobb literális típussá váljon. Ezt úgy kényszeríthetjük ki, hogy a szó szerinti érték végére egy __U__ vagy __L__ (_vagy mindkettő_) betűt teszünk:
...
:::danger Ezekre az utótagokra nem túl gyakran lesz szükségünk.
:::
...
A számok írásakor a szokásos konvenció szerint hármas csoportosításban (_ezrek, milliók, milliárdok stb._) írjuk őket, de a C# fordító nem törődik azzal, hogy ezek hol jelennek meg. Ha egy másik csoportosításnak több logikai értelme van, akkor használjuk úgy. A következők mindegyike megengedett:
...
A C#-ban a `char` típus egyetlen karaktert reprezentál, míg a `string` a tetszőleges hosszúságú szöveget.
...
Ha egy szám literális tartalmaz egy tizedespontot, akkor az egy `double` literális lesz, nem pedig egész literális. A végéhez `f` vagy `F` hozzáfűzése egy `float` literállá teszi. Az `m` vagy `M` hozzáfűzése pedig decimális literálissá alakítja azt.

Mind a három típus nagyobb tartományt képes reprezentálni, mint bármely egész típus, így ha egy egész literált használunk, a fordító automatikusan átkonvertálja azt.


## A bool típus

Az utolsó típus, amelyet itt tárgyalunk az a `bool` típus. A `bool` típus furcsának tűnhet, ha új vagy a programozásban, de hamarosan látni fogjuk az értékét. A `bool` típus a __Boole-algebráról__ kapta a nevét, amelyet _George Boole_, a tervezője után neveztek el. A `bool` típus igazságértékeket reprezentál. Ezeket a döntéshozatalban használjuk és két lehetséges értéke van: __true__ és __false__. Mindkettő `bool` literál, amit beleírhatunk a kódodba:
...
Ez csak akkor működik, ha a változót ugyanazon a soron inicializáljuk, ahol deklaráljuk. Ellenkező esetben nincs elegendő információ a fordító számára ahhoz, hogy következtessen a típusára. Például ez nem fog működni:
...
Láthatjuk, hogy a `Convert.ToInt32` metódusa szövegként vár bemenetet és egy egész számot ad eredményként, az átalakítás folyamatában a szöveget konvertálva. A `Convert` osztálynak vannak `To...` metódusai, amelyek lehetővé teszik a beépített típusok közötti konvertálást:
...
A `short`, `int` és `long` típusok az `Int` szót és a hozzátartozó bitméretet használják. Például ha `short` típust __16 bitet__ (_2 bájt_) használnánk, akkor a `ToInt16` használnánk. Az `ushort`, `uint` és `ulong` ugyanezt csinálja, csak az `UInt` kulcsszóval.

A másik meglepetés az, hogy egy lebegőpontos számra való átalakítás `ToSingle`, nem pedig `ToFloat`. 

A konzolablakból származó összes `input` szöveg típusú. Sok programnak szüksége lesz arra, hogy átalakítsuk a felhasználó bekért szöveget egy másik típussá annak érdekében, hogy később azzal dolgozhassunk. A szöveg elemzésének, felbontásának és más adattípussá történő átalakításának folyamata __parszolásnak__ (_parse_) nevezhető. A `Convert` osztály nagyszerű kiindulópont a szöveg __parszolásához__, bár idővel további parszolási eszközöket is tanulunk majd.
