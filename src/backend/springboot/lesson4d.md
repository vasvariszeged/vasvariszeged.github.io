...
::: note PagingAndSortingRepository
...
7. Most injektáljuk be az `OwnerRepository`-t a `DemoApplication` osztályba konstruktor injection segítségével:
...
3. Módosítanunk kell a `run` metódust tartalmát, hogy mentse a tulajdonosokat és az autókkat a kapcsolótáblánkba:
...
public void run(String... args) throws Exception {
    ...
    Owner owner1 = new Owner("John" , "Johnson");
    Owner owner2 = new Owner("Mary" , "Robinson");
    owner1.setCars(new HashSet<>(Arrays.asList(car1,car3)));
    owner2.setCars(new HashSet<>(Arrays.asList(car2,car3)));
    orepository.saveAll(Arrays.asList(owner1, owner2));
...
4. Most ha futtatjuk az alkalmazást létrejön egy új kapcsolótábla, amit __car_owner__-nek nevez el a programunk (`Car` és `Owner` táblák). A kapcsolótábla egy speciális típusú tábla, amely kezeli a __many-to-many__ kapcsolatot két tábla között.
